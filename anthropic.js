...
(function () {
  ...
  /** Sumber data tunggal; melempar bila data provider gagal dimuat. */
  function getModels() {
    const models = window.ANTHROPIC_MODELS;
    if (!Array.isArray(models)) {
      throw new Error(
        "window.ANTHROPIC_MODELS bukan array — data/anthropic_models.js kemungkinan gagal dimuat"
      );
    }
    return models;
  }
  ...
  function getStatusMeta(status) {
    ...
    if (!meta[status]) {
      logWarning("getStatusMeta", `status tidak dikenal: "${status}" — memakai "active"`);
      return meta.active;
    }
    return meta[status];
  }
  ...
  function openModal(id) {
    ...
    try {
      ...
      if (!m) throw new Error(`Model dengan id "${id}" tidak ditemukan pada data Anthropic`);
      ...
      if (content && overlay) {
        content.innerHTML = `
          <h2 style="color: #d97757;">Detail tidak tersedia</h2>
